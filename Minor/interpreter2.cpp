...
class MIPS {
    ...
    bool sw(int address,string reg,int i){
        ...
        //for stack the adress would be negative
            ...
            rowB=a;
            b=b/4;
            rowBuffer[b]=getRegValue(reg);
            waitTime.at(reg)=t;
            active.insert(active.begin(),reg);
            memIn.insert(memIn.begin()," | memory address "+to_string(address)+"-"+to_string(address+3)+"="+to_string(getRegValue(reg)));
    ...
    bool lw(int address, string reg,int i) {
        ...
        //for stack the sp value will be negative
            ...
            waitTime.at(reg)=t;
            active.insert(active.begin(),reg);
            memIn.insert(memIn.begin()," | "+reg+"="+to_string(rowBuffer[b]));
            cycleI+=", DRAM request issued and put in queue but waiting for response( has wait time of "+to_string(t)+" cycles.)\n";
            return true;            
        }             
    }
    ...
    void update(int i,string s,string info){
        if (s=="sw" || s=="lw"){
            ...
            waitTime.at(active.back())-=1;
            flag=false;
            if (waitTime.at(active.back())==0){
                cycleI+="cycle "+to_string(clock)+": Execute "+to_string(i)+" DRAM for "+active.back()+" completed"+memIn.back();
                active.pop_back();
                memIn.pop_back();
                flag=true;  
            }
            else {
                cycleI+="cycle "+to_string(clock)+": Execute "+to_string(i)+" along with DRAM for "+active.back();
            }   
            return;
        }
        else {
            ...
            if (waitTime.at(active.back())==0){
                ...
                cycleI+="cycle "+to_string(clock)+": Execute "+to_string(i)+" along with DRAM for "+active.back()+" |"+info;
            }   
            return;           
        }
    }
    ...
    bool executeInst(){
        ...
        while(i<n){ 
            ...
            if (v.size() ==0){
                ...
                if (ab=="add"){
                    string info;
                    bool a =waitTime.at(v.at(1))==0;
                    bool b=waitTime.at(v.at(3))==0;
                    bool c =waitTime.at(v.at(5))==0;
                    while (!(a && b && c)){
                        if (!active.empty()){
                            cycleI+="cycle "+to_string(clock+1)+"-"+to_string(clock+waitTime.at(active.back()))+": Only DRAM access is done and value is written/loaded to "+active.back()+" because instruction at "+to_string(i)+" is dependent on "+active.back()+memIn.back()+"\n";
                            clock +=waitTime.at(active.back());
                            waitTime.at(active.back())=0;
                            active.pop_back(); 
                            memIn.pop_back();    
                        } 
                        else {
        ...
        while (!active.empty()){
            cycleI+="cycle "+to_string(clock+1)+"-"+to_string(clock+waitTime.at(active.back()))+": Only DRAM access is done and value is written/loaded to "+active.back()+" because reached the end of program"+memIn.back()+"\n";
            clock +=waitTime.at(active.back());
            waitTime.at(active.back())=0;
            active.pop_back();   
            memIn.pop_back();
